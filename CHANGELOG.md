...
- Corrige falha ao carregar camadas do BDGEx em máquina cujo proxy do sistema tem os hosts do Exército na lista de exceção: a pilha de rede do QGIS entrega ao socket um proxy não resolvido e a conexão morre antes de abrir, com "The proxy type is invalid for this operation". O GetCapabilities passa a ser refeito com gerente de rede próprio e proxy resolvido a mão, e o contorno para o provedor WMS só é instalado depois que esse retry provar que a máquina tem o defeito, então máquina sadia não é tocada. As URLs dos três serviços passam a https, para onde o BDGEx redireciona de qualquer forma, e as falhas que antes eram mudas passam a ir para o log;
...
- Nova CLI (dsgtools_cli) para executar algoritmos de processamento do DSGTools via linha de comando, integrada ao qgis_process: validação local de parâmetros antes de rodar (nome inexistente com sugestão por proximidade, obrigatório ausente, índice de enum fora da faixa), describe compacto, modo --dry-run, cache de contrato em disco e doctor que diagnostica o provider (plugin ausente, desabilitado ou carregado sem nenhum algoritmo) e aponta o conserto;
- Novo algoritmo de identificação de gaps e overlaps em cobertura por XOR de bordas (edge XOR);
...
- Remoção de funcionalidades não utilizadas e de diversos códigos mortos (removida gestão de bancos no servidor, dsgtools_admindb, gestão e carregamento de estilo, etc);

Correções de bug:

- Corrige estatística de regras (Rule Statistics) no fluxo do SAP Operador: contexto de processamento passa a usar o projeto ativo para resolver as camadas de entrada;
- Corrige barra de revisão: campos de ranking/visitado tratados como vazios além de nulos, e trata falha ao salvar ao marcar feições como não visitadas;
- Corrige erro ao mesclar mais de uma camada no processo de desconstruir polígonos (Unbuild Polygons);
- Corrige erro ao montar estruturas auxiliares do grafo quando há trechos com geometria nula;
- Corrige validação de depressão e de ponto cotado na verificação de terreno (leitura do atributo de depressão, sentido do filtro de cota e mesclagem de curvas com atribuição divergente);
- Corrige geração de ponto cotado com valor múltiplo da equidistância no processo de extração de pontos cotados;
- Corrige generalização de curvas de nível: segmenta curvas fechadas antes de medir e filtrar (o filtro de tamanho mínimo media só a primeira parte da geometria), corrige a medição do perímetro em CRS geográfico e a atributação de índice e depressão, que saíam com valor fixo;
- Corrige validação do terreno recompondo curvas fechadas segmentadas em dois arcos antes de validar;
- Corrige vulnerabilidade de XXE na leitura de metadados XML do preparo de pacotes raster para o BDGEx e injeção de SQL na definição do SRID de bancos Geopackage;
- Corrige ícones do plugin não carregando (faltava registrar o módulo de recursos Qt) e menu "About DSGTools" desaparecendo no macOS;
