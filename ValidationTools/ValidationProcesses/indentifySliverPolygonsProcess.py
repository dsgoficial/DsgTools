...
class IdentifySliverPolygonsProcess(ValidationProcess):
    def __init__(self, postgisDb):
        super(self.__class__,self).__init__(postgisDb)
        self.parameters = {'Thickness': 0.3}
    ...
    def execute(self):
        ...
        try:
            ...
            if len(result.keys()) > 0:
                ...
                self.setStatus('%s features are sliver polygons. Check flags.\n' % numberOfProblems, 4) #Finished with flags
                QgsMessageLog.logMessage('%s features are sliver polygons. Check flags.\n' % numberOfProblems, "DSG Tools Plugin", QgsMessageLog.CRITICAL)
                return
            else:
                self.setStatus('There are no sliver polygons.\n', 1) #Finished
                QgsMessageLog.logMessage('There are no sliver polygons.\n', "DSG Tools Plugin", QgsMessageLog.CRITICAL)
