...
"""
Adapted from QGIS
https://github.com/qgis/QGIS/blob/master/python/plugins/processing/tests/AlgorithmsTestBase.py
***************************************************************************
    AlgorithmsTest.py
    ---------------------
    Date                 : January 2016
...
class AlgorithmsTest(object):
    def test_algorithms(self):
        ...
        with open(
            os.path.join(
                os.path.dirname(__file__), "tests_yaml", self.get_definition_file()
            ),
            "r",
        ) as stream:
    ...
    def filepath_from_param(self, param):
        ...
        if "uri" in param:
            ...
            path = os.path.join(prefix, param["location"])
            ...
            if expected_result["type"] in ("vector", "table"):
                ...
                if len(expected_lyrs) == 1:
                    self.assertLayersEqual(
                        ...
                        compare=compare,
                        pk=pk,
                        geometry={"topo_equal_check": topo_equal_check},
                    )
                else:
                    res = False
                ...
                self.assertTrue(
                    ...
                    "File does not exist: {}, {}".format(results[id], params),
                )
