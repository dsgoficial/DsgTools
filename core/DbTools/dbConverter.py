...
class DbConverter(QObject):
    ...
    def checkAndCreateDataset(self, conversionStepMap, feedback=None):
        """
    ...
    def prepareSpatialFilterLayer(self, spatialFilters, context=None, feedback=None):
        """
    ...
    def prepareInputLayers(self, inputLayers, stepConversionMap, context=None, feedback=None):
        ...
        spatialFilterlLayer = self.prepareSpatialFilterLayer(filters["spatial_filter"], context, feedback)
        preparedLayers = dict()
        for layer, vl in inputLayers.items():
            # if feedback is not None:
            #     feedback.setCurrentStep(currentInputStep)
            #     currentInputStep += 1
    ...
    def mapFeatures(self, inputPreparedLayers, outputLayers, featureConversionMap=None):
        """
        ...
        if featureConversionMap is not None:
            ...
            lh = LayerHandler()
            fh = FeatureHandler()
            for layer, vl in inputPreparedLayers.items():
                if vl.featureCount() == 0:
                    continue
    ...
    def loadToOuput(self, featuresMap, outputLayers, featureConversionMap=None):
        """
        ...
        success = dict()
        fail = dict()
        for layer, featureSet in featuresMap.items():
    ...
    def convertFromMap(self, conversionMap=None, featureConversionMap=None):
        ...
        if conversionMap is None:
            conversionMap = self.conversionMap
        conversionSteps = self.getConversionCount(conversionMap)
        allInputLayers = dict()
        ...
        for inputDb, conversionStepMaps in conversionMap.items():
            ...
            for conversionStepMap in conversionStepMaps:
                ...
                mappedFeatures = self.mapFeatures(preparedLayers, outputLayers)
                successfulLayers, failedLayers = self.loadToOuput(mappedFeatures, outputLayers)
        tests = (errors, successfulLayers, failedLayers)
        status = True
        log = "CONVERTEU TUDO!"
        return status, log, tests
