...
class DbConverter(QObject):
    ...
    def readInputLayers(self, conversionMap=None):
        ...
        inputLayerMap = dict()
        failedInputs = []
        for ds in self.getAllUniqueInputDb(conversionMap=conversionMap):
            ...
            for conversionStep in conversionSteps:
                ...
                for layer in layers:
                    if multiStepFeedback is not None:
                        ...
                        layerStep += 1
                    inputLayersMap["{0}::{1}".format(inputDb, conversionStep["outDs"])][layer] = \
    ...
    def readOutputLayers(self, conversionMap=None):
        ...
        outputLayerMap = dict()
        failedOutputs = []
        for ds in self.getAllUniqueOutputDb(conversionMap=conversionMap):
            parameters = self.parseDatasourcePath(ds)
            abstractDb = self.connectToDb(parameters=parameters)
            if abstractDb is None:
