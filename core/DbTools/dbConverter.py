...
class DbConverter(QObject):
    ...
    def createDataset(self, parameters):
        ...
        driverName = {
            DsgEnums.DriverPostGIS : "QPSQL",
            DsgEnums.DriverSpatiaLite : "QSQLITE",
            DsgEnums.DriverGeopackage : "GPKG",
            DsgEnums.DriverShapefile : "SHP"
        }[parameters['driver']]
        DbCreatorFactory().createDbCreatorFactory(driverName=driverName, createParam=parameters["db"])

    def checkAndCreateDatabases(self, conversionMap=None):
        """
            ...
            for conversionStep in conversionSteps:
                ...
                if conversionStep["createDs"] == "True" and not (output in success or output in fail):
                    try:
                        ...
                        parameters['crs'] = conversionStep['crs']
                        parameters['edgv'] = conversionStep['edgv']
                        success[output] = self.createDataset(parameters=parameters)
                    except Exception as e:
                        fail[output] = "{0} dataset has failed: {1}".format(output, "; ".join(e.args))
        return success, fail
    ...
    def prepareLayers(self, conversionMap=None, context=None, feedback=None):
        """
