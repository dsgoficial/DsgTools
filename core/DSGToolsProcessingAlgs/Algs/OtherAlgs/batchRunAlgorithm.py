...
class BatchRunAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterString(
                self.ALG_NAME,
                self.tr('Name of the algorithm with provider')
            )
        )
        self.addParameter(
            QgsProcessingParameterString(
                self.INPUT_LAYER_PARAMETER_NAME,
                self.tr('Name of the key of the input')
            )
        )
        self.addParameter(
            QgsProcessingParameterString(
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        algParameterDict = self.loadAlgorithmParametersDict(parameters, context, feedback)
        algName = self.parameterAsString(
            parameters,
            self.ALG_NAME,
            context
        )
    ...
    def loadAlgorithmParametersDict(self, parameters, context, feedback):
        rules_text = self.parameterAsString(
            parameters,
            self.PARAMETER_DICT,
            context
        )
