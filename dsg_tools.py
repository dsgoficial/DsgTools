...
class DsgTools:
    ...
    def showAbout(self):
        ...
        # closed linestrings
        lineGeom = QgsGeometry.fromWkt( "LineString(0.1 -0.1, 10.1 0, 9.9 10.1, 0 10, 0.1 -0.1)")
        result = snapper.snapGeometry( lineGeom, 1 )
        print 'saida   ', result.exportToWkt()
        print 'esperado', "LineString (0 0, 10 0, 10 10, 0 10, 0 0)"
        print

        # rl = QgsVectorLayer("MultiPolygon", "x", "memory")
        # ff = QgsFeature()
        # refGeom = QgsGeometry.fromWkt("MultiPolygon (((806045.34690000023692846 8588848.34759999997913837, 806228.28419999964535236 8588846.35940000042319298, 806239.57679999992251396 8588823.79680000059306622, 806111.42009999975562096 8588816.49120000004768372, 806020.97339999955147505 8588743.53610000014305115, 805994.41610000003129244 8588656.83880000002682209, 805963.44849999994039536 8588557.1415999997407198, 805872.71949999965727329 8588458.09359999932348728, 805787.06269999966025352 8588432.9282000008970499, 805710.65160000044852495 8588472.90139999985694885, 805647.70710000023245811 8588573.61800000071525574, 805605.96399999968707561 8588661.05660000070929527, 805581.29550000000745058 8588748.30980000086128712, 805581.95600000023841858 8588809.19250000081956387, 805577.870299999602139 8588853.42290000058710575, 806045.34690000023692846 8588848.34759999997913837)))")
        # ff.setGeometry(refGeom)
        # flist = []
        # flist.append(ff)
        # rl.dataProvider().addFeatures(flist)
        ...
        # polygonGeom = QgsGeometry.fromWkt("MultiPolygon (((805789.40439178317319602 8588854.81971519626677036, 805969.59786686196457595 8589469.69529631175100803, 806033.63585569534916431 8588849.41391094215214252, 805789.40439178317319602 8588854.81971519626677036)))" )
        # result = snapper.snapGeometry( polygonGeom, 1000)
        # print 'saida   ', result.exportToWkt()
        # print
        ...
        ff = QgsFeature()
        refGeom = QgsGeometry.fromWkt("Linestring(0 0, 10 1, 20 0)") 
        ff.setGeometry(refGeom)
        flist = []
        flist.append(ff)
        rl.dataProvider().addFeatures(flist)
