...
def update_translation_pro(pro_file, py_files, ui_files):
    ...
    try:
        ...
        translations_line = translations_match.group(0) if translations_match else "TRANSLATIONS = i18n/DsgTools_pt.ts"
        
        # Extract any other content after TRANSLATIONS (e.g., RESOURCES)
        resources_match = re.search(r'(RESOURCES\s*[^\n]+).*$', content, re.DOTALL)
        resources_line = resources_match.group(1) if resources_match else "RESOURCES += resources.qrc"
...
def main():
    parser = argparse.ArgumentParser(description='Update translation.pro file for QGIS plugin')
    parser.add_argument('--dir', default='.', help='Plugin directory (default: current directory)')
    parser.add_argument('--output', default='translation.pro', help='Output pro file path (default: translation.pro)')
