...
class ButtonEditorWidget( TableEditorWidget ):
    ...
    def createEditRowWidget(self, row, col):
        ...
        cloneBtn.clicked.connect(
            lambda *args, index=index: self.handleCloneBtn(index)
        )
    ...
    def addRow(self, 
        ...
        self.tableWidget.setItem(idx, 3, self.createNotEditableItem( json.dumps( buttonConfig ) ))
    ...
    def getRowIndex(self, buttonId):
        ...
        for idx in range(self.tableWidget.rowCount()):
            if not (
                    buttonId == self.tableWidget.model().index(idx, 0).data()
                ):
                continue
            return idx
        return -1
    ...
    def updateButton(self, newData, oldData, callback):
        ...
        self.addRow( newData['buttonId'], newData['buttonTabName'], newData['buttonName'], newData )
