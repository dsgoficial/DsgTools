...
class CreateGridAlongLineAlgorithm(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        margins_bottom = self.parameterAsDouble(
            parameters, self.MARGINS_BOTTOM, context
        )
        ...
        # Tamanho da folha
        if paper_size == 0:
            proportion = (297, 210)  # proporção do a4 Altura/largura
        else:
            proportion = (420, 297)  # proporção do a3
    ...
    def calcula_grid(self, source, sink, start, width, height, overlap, feedback):
        ...
        for current, feature in enumerate(source.getFeatures()):
            ...
            numpages = (
                geomlength / width
            )  # Divisão do comprimento da feição pela altura da folha
            step = 1.0 / numpages
            ...
            while curs <= 1:
                ...
                geom = self.createPolygon(
                    x_start, y_start, startpoint, endpoint, height, width
                )
                curs = curs + stepnudge
