...
class RaiseFlagsAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config):
        ...
        self.addParameter(
            QgsProcessingParameterString(self.TABLE_SCHEMA, self.tr("Table schema"))
        )
        self.addParameter(
            QgsProcessingParameterString(self.TABLE_NAME, self.tr("Table name"))
        )
        self.addParameter(
            QgsProcessingParameterString(
                self.GEOMETRY_COLUMN, self.tr("Geometry column")
            )
        )
        self.addParameter(
            QgsProcessingParameterString(
                self.OUTPUT_FLAG_TEXT_FIELD, self.tr("Flag text field")
            )
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        geometryColumn = self.parameterAsString(
        ...
        flagTextAttr = self.parameterAsString(
            parameters, self.OUTPUT_FLAG_TEXT_FIELD, context
        )
        ...
        outputLyr = self.getOutputFromInput(
            inputLyr, tableSchema, tableName, geometryColumn
        )
