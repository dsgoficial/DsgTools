...
class AssignDefaultFieldValueToLayersAlgorithm(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if os.path.exists(inputJSONFile):
            self.loadDefaultFieldValueFromJSONFile(
                inputJSONFile, inputLyrList, feedback
            )
        elif len(inputJSONData) > 0:
            self.loadDefaultFieldValueFromJSONData(
                inputJSONData, inputLyrList, feedback
            )
        else:
            return {self.OUTPUT: []}
    ...
    def tr(self, string):
        return QCoreApplication.translate(
            "AssignDefaultFieldValueToLayersAlgorithm", string
        )
