...
class AssignFormatRulesToLayersAlgorithm(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        inputLyrList = [
            lyr for lyr in inputLyrList if lyr.dataProvider().name() == "postgres"
        ]
        if not inputLyrList:
            return {}
            ...
            for lyr in ruleDict:
                for attribute in ruleDict[lyr]:
                    ruleDict[lyr][attribute] = sorted(
                        ruleDict[lyr][attribute], key=itemgetter("rank"), reverse=False
    ...
    def addRuleToLayer(self, lyr, feedback=None):
        ...
        for field in lyr.fields():
            if feedback is not None and feedback.isCanceled():
                break
            if key not in self.ruleDict or field.name() not in self.ruleDict[key]:
                continue
            fieldStyleList = [
    ...
    def createRuleVirtualField(self, lyr):
        ...
        sortedRuleList = sorted(
            ruleList, key=itemgetter("rank", "atributo"), reverse=False
        )
    ...
    def expressionHasParseError(self, expressionString):
        expr = (
            expressionString
            if """ELSE ''\nEND""" in expressionString
            else expressionString + """ELSE ''\nEND"""
        )
