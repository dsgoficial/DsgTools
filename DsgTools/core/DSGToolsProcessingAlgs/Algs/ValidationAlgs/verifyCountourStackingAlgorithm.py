...
class VerifyCountourStackingAlgorihtm(ValidationAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        levelsField = self.parameterAsFields(parameters, "INPUT_LEVES_FIELD", context)[
            0
        ]
    ...
    def compareLevel(
        ...
        for feature1 in countourLayerPoly.getFeatures():
            ...
            feedback.setProgress(
                step * (1 + (auxstep / auxProgressStep)) * progressStep
            )
            toCompare = []
            areaComp = []
            skip = True
            ...
            if fToCompare[isDepressionField] == isNotDep:
                if feature1[isDepressionField] == isNotDep:
                    if not (
                        (feature1[levelsField] - fToCompare[levelsField]) == levelGap
                    ):
            ...
            if fToCompare[isDepressionField] == isDep:
                if feature1[isDepressionField] == isDep:
                    if not (
                        (fToCompare[levelsField] - feature1[levelsField]) == levelGap
                    ):
    ...
    def outLayer(self, parameters, context, polygons, streamLayer):
        ...
        dicterro = {
            1: self.tr(
                "Extern and intern are normal, but elevation differences are not greater than one equidistance"
            ),
            2: self.tr(
                "Extern and intern are depressions, but elevation differences are not smaller than one equidistance"
            ),
            3: self.tr(
                "Extern is normal, intern is depression, but elevations are different"
            ),
            4: self.tr(
                "Extern is depression, intern is normal, but elevations are different"
            ),
        }
