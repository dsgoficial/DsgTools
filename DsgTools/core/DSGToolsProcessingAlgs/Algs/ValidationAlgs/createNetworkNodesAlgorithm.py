...
class CreateNetworkNodesAlgorithm(ValidationAlgorithm):
    ...
    def initAlgorithm(self, config):
        ...
        self.addParameter(
            QgsProcessingParameterFeatureSink(self.NETWORK_NODES, self.tr("Node layer"))
        )
        self.addParameter(
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        (nodeSink, dest_id) = self.parameterAsSink(
            parameters,
            self.NETWORK_NODES,
            context,
            self.getFields(),
        ...
        frame = layerHandler.getFrameOutterBounds(
            frameLayer, algRunner, context, feedback=multiStepFeedback
        )
        currStep += 1
        # get search radius
        ...
        self.nodeDict = networkHandler.identifyAllNodes(
            networkLayer=networkLayer, feedback=multiStepFeedback
        )  # zoado, mudar lógica
        multiStepFeedback.pushInfo(
            self.tr("{node_count} node(s) identificated...").format(
                node_count=len(self.nodeDict)
            )
        )
        currStep += 1
        # new step
