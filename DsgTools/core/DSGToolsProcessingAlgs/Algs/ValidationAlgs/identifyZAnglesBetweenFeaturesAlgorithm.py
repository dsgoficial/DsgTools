...
class IdentifyZAnglesBetweenFeaturesAlgorithm(ValidationAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        for feat in inputSource.getFeatures():
            ...
            if total_cached % 10000 == 0:
                feedback.pushInfo(self.tr(f"Cached {total_cached}/{feature_count} features..."))
        ...
        multiStepFeedback = QgsProcessingMultiStepFeedback(nSteps, feedback) if nSteps > 1 else feedback
        currentStep = 0

        # Process based on geometry type
        if geometry_type == QgsWkbTypes.LineGeometry:
            # First process Z angles within each line
            multiStepFeedback.setProgressText(self.tr("Evaluating Z angles within lines"))
            multiStepFeedback.setCurrentStep(currentStep)
            
            # Process all features using concurrently
            for result in concurrently(
                lambda feat_id: self.process_internal_line(feat_id, angle, inputSource.sourceName()),
            ...
            currentStep += 1
            multiStepFeedback.setProgressText(self.tr("Evaluating Z angles between features"))
            multiStepFeedback.setCurrentStep(currentStep)
            
            # Process line intersections
            between_lines_results = self.caseBetweenLines(inputSource, angle, feedback=multiStepFeedback)
            featsToAnalyse.extend(between_lines_results)
            
        else:  # Polygon geometry
            multiStepFeedback.setProgressText(self.tr("Evaluating Z angles within polygons"))
            
            # Process all features using concurrently
            for result in concurrently(
                lambda feat_id: self.process_internal_area(feat_id, angle, inputSource.sourceName()),
                self.feature_cache.keys(),  # Pass feature IDs instead of features
                feedback=multiStepFeedback
            ):
                if result:
    ...
    def process_internal_line(self, feat_id, angle, source_name):
        ...
        for i in range(len(all_vertices) - 3):
            ...
            angle1 = math.degrees(QgsGeometryUtils.angleBetweenThreePoints(
                v1.x(), v1.y(), v2.x(), v2.y(), v3.x(), v3.y()
            ))
            angle2 = math.degrees(QgsGeometryUtils.angleBetweenThreePoints(
    ...
    def caseBetweenLines(self, lines, angle, feedback=None):
        ...
        def process_line_pair(pair_data):
            ...
            if intersection.wkbType() == QgsWkbTypes.Point:
                ...
                _, g1VertexIdx, g1PreviousVertexIdx, g1NextVertexIdx, _ = gfeat1.closestVertex(intersection_point)
                _, g2VertexIdx, g2PreviousVertexIdx, g2NextVertexIdx, _ = gfeat2.closestVertex(intersection_point)
                ...
                if g1NextVertexIdx == g2PreviousVertexIdx == -1 and g1PreviousVertexIdx > 0 and g2NextVertexIdx < len(vg2) - 1:
                ...
                elif g2NextVertexIdx == g1PreviousVertexIdx == -1 and g2PreviousVertexIdx > 0 and g1NextVertexIdx < len(vg1) - 1:
                    # Create sequence of 4 points that might form a Z angle
                    points = [
                ...
                for feat3_id in candidate_ids:
                    ...
                    if gfeat3.touches(gfeat1):
                        ...
                        three_way_results = self._check_three_way_intersection(
                            gfeat1, gfeat2, gfeat3, feat1_id, feat2_id, feat3_id, lines.sourceName()
                        )
        ...
        feedback.pushInfo(self.tr(f"Processing {len(potential_pairs)} potential feature pairs"))
        for result in concurrently(process_line_pair, potential_pairs, feedback=feedback):
            if result:
                featsToAnalyse.extend(result)
        
        return featsToAnalyse

    def _check_three_way_intersection(self, g1, g2, g3, feat1_id, feat2_id, feat3_id, source_name):
        ...
        if inter12.wkbType() != QgsWkbTypes.Point or inter23.wkbType() != QgsWkbTypes.Point:
            return []
        ...
        if g1NextVertexIdx == g3PreviousVertexIdx == -1:
            ...
            if (g1PreviousVertexIdx >= 0 and g1VertexIdx >= 0 and 
                g3VertexIdx >= 0 and g3NextVertexIdx >= 0):
        ...
        # Case 2: Reverse configuration
            ...
            # Only process if we have valid vertex indices
            if (g3PreviousVertexIdx >= 0 and g3VertexIdx >= 0 and 
                g1VertexIdx >= 0 and g1NextVertexIdx >= 0):
    ...
    def _is_z_angle(self, angle1, angle2):
        ...
        return ((angle1 > self.angle_threshold and angle2 < self.angle_threshold_complement) or 
