...
class GeneralizeWaterBodyAlgorithm(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        water_body_layer = self.parameterAsVectorLayer(
        ...
        dissolve = algRunner.runDissolve(
            localRiversCache, context=context, feedback=multiStepFeedback
        )
        currentStep += 1
        multiStepFeedback.setCurrentStep(currentStep)

        multiStepFeedback.setProgressText(
            self.tr("Applying negative buffer to remove thin parts.")
        )
        buffer_neg = algRunner.runBuffer(
            dissolve,
        ...
        singlepart = algRunner.runMultipartToSingleParts(
            buffer_neg, context=context, feedback=multiStepFeedback
        )
        currentStep += 1
        multiStepFeedback.setCurrentStep(currentStep)

        multiStepFeedback.setProgressText(self.tr("Removing null geometries."))
        removenull = algRunner.runRemoveNull(
            singlepart, context=context, feedback=multiStepFeedback
        )
        currentStep += 1
        multiStepFeedback.setCurrentStep(currentStep)

        multiStepFeedback.setProgressText(self.tr("Applying positive buffer."))
        buffer_pos = algRunner.runBuffer(
            removenull,
            distance=min_water_body_width_tolerance / 2.0,
            context=context,
            dissolve=True,
        ...
        buffer_pos_single = algRunner.runMultipartToSingleParts(
            buffer_pos, context=context, feedback=multiStepFeedback
        )
        currentStep += 1
        multiStepFeedback.setCurrentStep(currentStep)

        multiStepFeedback.setProgressText(
            self.tr("Applying difference between original and buffer.")
        )
        difference_original = algRunner.runDifference(
            inputLyr=localRiversCache,
        ...
        new_singlepart = algRunner.runMultipartToSingleParts(
            difference_original, context=context, feedback=multiStepFeedback
        )
        currentStep += 1
        multiStepFeedback.setCurrentStep(currentStep)
        ...
        multiStepFeedback.setProgressText(
            self.tr("Applying difference between original and holes filtered.")
        )
        waterbodystretch = algRunner.runDifference(
            inputLyr=localRiversCache,
            overlayLyr=filtered_difference,
            context=context,
            feedback=multiStepFeedback,
        )
        currentStep += 1
        ...
        unified_single = algRunner.runMultipartToSingleParts(
        ...
        multiStepFeedback.setProgressText(self.tr("Creating spatial index."))
        algRunner.runCreateSpatialIndex(
            inputLyr=waterbody_empty_space, context=context, feedback=multiStepFeedback
        )
        currentStep += 1
        multiStepFeedback.setCurrentStep(currentStep)
        ...
        empty_spaces = algRunner.runMultipartToSingleParts(
            empty_spaces_merged, context=context, feedback=multiStepFeedback
        )
        currentStep += 1
        multiStepFeedback.setCurrentStep(currentStep)
        ...
        for drainage_feature in drainage_lines_layer.getFeatures():
            ...
            contained = False
            for water_body_id in water_body_index.intersects(
                drainage_geom.boundingBox()
            ):
    ...
    def shortHelpString(self):
        return self.tr(
            "Generalizes water bodies, islands, drainage segments and dams.\nFlow types expression: Choose the numbers related to rivers that have flow in the attribute table. Example: 'tipo in (1,2,9,10)'.\nScale: Choose the desired scale. Example: '50' for 50k.\nMinimum waterbody width tolerance: Choose the minimum water body width in degrees. Example: '0.000008' for 0.8mm.\nMinimum waterbody area tolerance: Choose the minimum water body area in square degrees. Example: '0.0000000004' for 4mm².\nMinimum island area tolerance: Choose the minimum island area in square degrees. Example: '0.0000000004' for 4mm².\nMinimum drainage line width tolerance: Choose the minimum drainage segment width in degrees. Example: '0.000008' for 0.8mm.\nSelect field from Drainage Lines to classify outside polygons: Choose the attribute table column related to the drainage line status inside the polygon to change it to outside the polygon.\nValue for outside polygon: Choose the numeric value related to the previously selected column that refers to elements outside the polygon.\nValue for secondary lines (inside polygons): Choose the value related to the previously selected column that refers to elements inside the polygons (secondary drainage lines).\nReference layer: Choose the reference layer for the Generalize Network Edges With Length plugin.\nPoint constraint layers: Choose the layers related to point constraints for the Generalize Network Edges With Length plugin.\nLine constraint layers: Choose the layers related to line constraints for the Generalize Network Edges With Length plugin.\nPolygon constraint layers: Choose the layers related to polygon constraints for the Generalize Network Edges With Length plugin."
        )
