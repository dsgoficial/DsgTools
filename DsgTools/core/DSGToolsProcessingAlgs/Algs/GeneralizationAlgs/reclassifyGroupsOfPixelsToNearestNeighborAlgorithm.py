...
class ReclassifyGroupsOfPixelsToNearestNeighborAlgorithm(ValidationAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        (
            ...
            sizeThresholds,
            nonGrowingClasses,
        ) = self.parseGeneralizationRules(rulesJson, min_area, feedback)
        ...
        if len(thresholdPhases) > 1:
            multiStepFeedback.pushInfo(
                self.tr("Processing in {0} phases based on size thresholds").format(
                    len(thresholdPhases)
                )
            )
            for idx, (threshold, dnList) in enumerate(thresholdPhases):
                if dnList == ["default"]:
                    multiStepFeedback.pushInfo(
                        self.tr(
                            "  Phase {0}: All other classes with threshold {1} m\u00b2"
                        ).format(idx + 1, threshold)
                    )
                else:
                    multiStepFeedback.pushInfo(
                        self.tr(
                            "  Phase {0}: Classes {1} with threshold {2} m\u00b2"
                        ).format(idx + 1, dnList, threshold)
                    )
        ...
        for phaseIdx, (threshold, dnList) in enumerate(thresholdPhases):
            ...
            if len(thresholdPhases) > 1:
                multiStepFeedback.pushInfo(
                    self.tr(
                        "Phase {0}/{1}: Processing {2} with threshold {3} m\u00b2"
                    ).format(
                        phaseIdx + 1,
                        len(thresholdPhases),
                        "classes " + str(dnList)
    ...
    def parseGeneralizationRules(
            ...
            for key, value in rawRestrictions.items():
                try:
                    ...
                    feedback.pushWarning(
                        self.tr(
                            "Skipping invalid class restriction: {0} -> {1}: {2}"
                        ).format(key, value, str(e))
                    )

            classRestrictions = self.makeBidirectional(intRestrictions)
            feedback.pushInfo(
                self.tr("Loaded class restrictions (bidirectional): {0}").format(
                    classRestrictions
                )
            )
            ...
            for key, value in rawThresholds.items():
                try:
                    ...
                    if key == "default":
                        ...
                        feedback.pushWarning(
                            self.tr(
                                "Skipping invalid size threshold: {0} -> {1}"
                            ).format(key, value)
                        )
        ...
        if "non_growing_classes" in rules:
            ...
            try:
                ...
                feedback.pushWarning(
                    self.tr("Invalid non-growing classes specification: {0}").format(
                        str(e)
                    )
                )
    ...
    def processPhase(
        ...
        feedback.pushInfo(
            self.tr("Found {0} polygons matching criteria").format(nFeats)
        )
        ...
        if polygons_sink is None and phaseIdx == 0:
            ...
            if (
                self.RECLASSIFIED_POLYGONS in parameters
                and parameters[self.RECLASSIFIED_POLYGONS]
            ):
        ...
        while nIterations < maxIterations:
            ...
            if remainingFeatCount == 0:
                feedback.pushInfo(
                    self.tr("No more polygons to process after {0} iterations").format(
                        nIterations
                    )
                )
                break
            ...
            if nIterations > 0:
                feedback.pushInfo(
                    self.tr("Iteration {0}: Processing {1} polygons").format(
                        nIterations + 1, remainingFeatCount
                    )
                )
    ...
    def processPixelGroupInPlace(
        ...
        maskedCurrentView = ma.masked_array(maskedCurrentView, viewArray == v, np.int16)
        ...
        if classRestrictions and v in classRestrictions:
            ...
            testMask1 = self._applyRestrictionMask(
                maskedCurrentView, viewArray, allowed_targets, nonGrowingClasses
            )
            ...
            if self._hasValidNeighbors(testMask1):
                ...
                if nonGrowingClasses:
                    growing_targets = [
                        t for t in allowed_targets if t not in nonGrowingClasses
                    ]
                    non_growing_excluded = len(growing_targets) > 0 and len(
                        growing_targets
                    ) < len(allowed_targets)
        ...
        if not restriction_applied and nonGrowingClasses:
            testMask2 = self._applyNonGrowingMask(
                maskedCurrentView, viewArray, nonGrowingClasses
            )
        ...
        reclassified_values = maskedCurrentView[~maskedCurrentView.mask][
            nearest_indices
        ]
        ...
        if non_growing_excluded and nonGrowingClasses:
            ...
            if restriction_applied:
                ...
                non_growing_pixels = non_growing_pixels & ~np.isin(
    ...
    def _applyRestrictionMask(
        ...
        if nonGrowingClasses:
            ...
            if growing_targets:
                ...
                testMaskWithoutNonGrowing = ma.masked_array(
                    testMask, growing_mask, dtype=np.int16
                )
