...
class SplitLinesAtMaximumLengthAlgorithm(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if closed_lines.featureCount() > 0:
            multiStepFeedback.pushInfo(f"Processing {closed_lines.featureCount()} closed lines...")
            
            # Process closed lines one by one to determine length correctly
            total = 100.0 / closed_lines.featureCount() if closed_lines.featureCount() else 0
            ...
            for current, feature in enumerate(closed_lines.getFeatures()):
                ...
                if line_length > max_length:
                    ...
                    vertices = geom.asMultiPolyline()[0] if geom.isMultipart() else geom.asPolyline()
                    if len(vertices) < 4:
                    ...
                    part2 = vertices[mid_idx:]  # Second half including midpoint (to end/start)
    ...
    def shortHelpString(self):
        ...
        """
        return self.tr("""This algorithm splits lines based on maximum length.
