...
class BuildTerrainSlicingFromContoursAlgorihtm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterRasterDestination(
                self.OUTPUT_RASTER, self.tr("Output slicing")
            )
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        (output_sink, output_sink_id) = self.getOutputSink(
            inputRaster, outputFields, parameters, context
        )
        ...
        geographicBounds = (
            ...
            if areaWithoutInformationSource is not None
            and areaWithoutInformationSource.featureCount() > 0
            else parameters[self.GEOGRAPHIC_BOUNDARY]
        )
        ...
        geographicBounds = (
            ...
            if waterBodiesSource is not None and waterBodiesSource.featureCount() > 0
            else geographicBounds
        )
        currentStep += 1
        ...
        algRunner.runCreateSpatialIndex(
            overlayedPolygons, context, feedback=multiStepFeedback
        )
        currentStep += 1
        ...
        for current, feat in enumerate(overlayedPolygons.getFeatures()):
            ...
            newFeat["class_min"], newFeat["class_max"] = slicingThresholdDict[
                feat["a_DN"]
            ]
    ...
    def overlayPolygonLayer(
        self, inputLyr, polygonLyr, crs, context, feedback, operator=0
    ):
        parameters = {
            "ainput": inputLyr,
            "atype": 0,
    ...
    def tr(self, string):
        return QCoreApplication.translate(
            "BuildTerrainSlicingFromContoursAlgorihtm", string
        )
    ...
    def shortHelpString(self):
        return self.tr(
            "O algoritmo constrói o fatiamento do terreno baseado nas curvas de nível."
        )
