...
class ETCQDGSegmentationEvaluator(QgsProcessingAlgorithm):
    ...
    def tr(self, string):
        return QCoreApplication.translate("ETCQDGSegmentationEvaluator", string)
    ...
    def displayName(self):
        return self.tr("ET-CQDG Segmentation Evaluator")
    ...
    def shortHelpString(self):
        return self.tr(
            """
        Generates segmentation masks and evaluates metrics by comparing them with inferences from multiple experiments.
        ...
        - class 0 = Background (included in the metrics)
        ...
        - {experiment}/metrics/: Evaluation metrics CSVs

        Consolidated (destination folder root):
        - consolidated_all_metrics.csv: Overall metrics for all experiments
        - per_class_metrics.csv: Per-class metrics for all experiments
        - consolidated_tile_metrics.csv: Metrics for all tiles across all experiments

        For each MI:
        1. Computes combined extent and clips the inferred mask
        2. Reprojects to the corresponding UTM zone
        3. Uses the pixel size from the reprojected raster

        For each tile:
        1. Extracts intersecting polygons
        2. Generates ground truth (rasterization)
        3. Clips tile from the inferred mask
        4. Computes metrics: Accuracy, IoU, Precision, Recall, F1
        5. Computes per-class metrics (including Background)
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        segmentation_rasters = self.parameterAsLayerList(
        ...
        # 3. Mapear Classes
        class_names = {}
        has_class_name_field = class_name_field and class_name_field in [
            f.name() for f in mask_source.fields()
        ]
            ...
            if class_value is not None and class_value not in class_names:
                if has_class_name_field:
                    ...
                    class_names[int(class_value)] = (
                        str(class_name) if class_name else f"Classe_{class_value}"
                    )
                else:
        ...
        mask_layer_ref = context.getMapLayer(
            mask_source.sourceName()
        ) or QgsVectorLayer(mask_source.sourceName(), "mask_ref", "ogr")
    ...
    def _processExperiment(
        ...
        for tile_feat in tiles_source.getFeatures():
            tiles_by_mi[tile_feat["mi"]][
                f"{tile_feat['mi']}_{tile_feat['quadricula']}"
            ] = tile_feat
            total_tiles_count += 1
        ...
        mi_accumulators = defaultdict(
            lambda: {"counts": {}, "total_pixels": 0, "correct_pixels": 0}
        )
        metrics_by_tile = []
            ...
            for mi, tile_feature_dict in tiles_by_mi.items():
                ...
                if not utm_crs.isValid():
                    multiStepFeedback.reportError(
                        self.tr("Invalid UTM zone. Skipping MI {0}.").format(mi)
                    )
                    current_step_index += 1 + len(tile_features)
                    continue

                # 1. Criar camada temporária com TODOS os tiles deste MI
                mi_tiles_layer = QgsVectorLayer(
                    f"Polygon?crs={tiles_crs.authid()}", f"tiles_{mi}", "memory"
                )
                ...
                reprojected_tiles_layer = algRunner.runReprojectLayer(
                    layer=mi_tiles_layer,
                    targetCrs=utm_crs,
                    context=context,
                    feedback=multiStepFeedback,
                )
                algRunner.runCreateSpatialIndex(
                    reprojected_tiles_layer, context=context
                )

                # Recortar e Reprojetar Máscara Global para o MI
                mi_extent_utm = reprojected_tiles_layer.extent()
                xform_utm_to_mask = QgsCoordinateTransform(
                    utm_crs, mask_source.sourceCrs(), QgsProject.instance()
                )
                mi_extent_mask_crs = xform_utm_to_mask.transformBoundingBox(
                    mi_extent_utm
                )

                mask_clipped_layer = algRunner.runExtractByExtent(
                    inputLayer=mask_layer_ref,
                    extent=mi_extent_mask_crs,
                    context=context,
                    feedback=multiStepFeedback,
                )

                reprojected_mask_layer = algRunner.runReprojectLayer(
                    layer=mask_clipped_layer,
                    targetCrs=utm_crs,
                    context=context,
                    feedback=multiStepFeedback,
                )
                ...
                for feat in intersected_layer.getFeatures():
                    ...
                    if v_class is not None and v_quad:
                        ...
                        try:
                            ...
                            gt_shapes_by_tile[tile_id_key].append(
                                (geom_json, int(v_class))
                            )
                        except:
                            pass
                ...
                if utm_crs != seg_raster_crs:
                    xform_utm_to_raster = QgsCoordinateTransform(
                        utm_crs, seg_raster_crs, QgsProject.instance()
                    )
                    extent_raster_crs = xform_utm_to_raster.transformBoundingBox(
                        combined_extent
                    )
                else:
                ...
                buf_x, buf_y = (
                    extent_raster_crs.width() * 0.1,
                    extent_raster_crs.height() * 0.1,
                )
                ...
                # --- LOOP DE SUBMISSÃO ---
                multiStepFeedback.pushInfo(
                    self.tr("Submitting tiles from MI {0} to threads...").format(mi)
                )
                ...
                for tile_feat_utm in reprojected_tiles_layer.getFeatures():
                    ...
                    try:
                        ...
                        multiStepFeedback.reportError(
                            self.tr("Tile error {0}: {1}").format(quadricula, e)
                        )
            ...
            for future in concurrent.futures.as_completed(futures_map):
                ...
                try:
                    ...
                    if result["status"] == "success":
                        ...
                        t_px, c_px = m.get("total_pixels", 0), m.get(
                            "correct_pixels", 0
                        )
                        ...
                        if tile_id in tile_data_cache:
                            ...
                            for cid in sorted_classes:
                                ...
                                new_feat.setAttribute(
                                    f"{dest_prefix}_iou", m.get(f"{src_prefix}_iou")
                                )
                                new_feat.setAttribute(
                                    f"{dest_prefix}_f1", m.get(f"{src_prefix}_f1_score")
                                )
                                new_feat.setAttribute(
                                    f"{dest_prefix}_prec",
                                    m.get(f"{src_prefix}_precision"),
                                )
                                new_feat.setAttribute(
                                    f"{dest_prefix}_rec", m.get(f"{src_prefix}_recall")
                                )
        ...
        global_metrics.update(
            {"num_tiles": processed_count, "num_mis": len(mi_accumulators)}
        )
        ...
        self._write_csv_safe(
            os.path.join(metrics_folder, "metrics_per_tile.csv"), metrics_by_tile
        )
        ...
        self._write_csv_safe(
            os.path.join(metrics_folder, "metrics_per_mi.csv"), mi_list
        )

        self._write_csv_safe(
            os.path.join(metrics_folder, "metrics_overall.csv"), [global_metrics]
        )
    ...
    def _write_csv_safe(self, path, data_list):
        ...
        header = [k for k in priority if k in all_keys] + sorted(
    ...
    def _writeConsolidatedMetrics(
        ...
        self._write_csv_safe(
            os.path.join(output_folder, "per_class_metrics.csv"), per_class_consolidated
        )

        # Tile metrics consolidados (todos os tiles de todos experimentos)
        self._write_csv_safe(
            os.path.join(output_folder, "consolidated_tile_metrics.csv"), tile_metrics
        )

        # MI metrics consolidados
        self._write_csv_safe(
            os.path.join(output_folder, "consolidated_mi_metrics.csv"), mi_metrics_list
        )
    ...
    try:
        ...
        with rasterio.open(reprojected_raster_path) as src:
            out_image, out_transform = mask(
                src, [tile_mask_geom], crop=True, nodata=nodata
            )
        ...
        metrics = calculateSegmentationMetrics(
            gt_path, predicted_path, nodata, class_names
        )
