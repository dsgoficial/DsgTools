...
um problema do BDGEx nem deste plugin: medido no QGIS 3.44 e no 4.2, com um
QTcpSocket cru, os hosts que o sistema manda ir direto falham todos com
"The proxy type is invalid for this operation" e nem abrem o socket, e os
que devem passar pelo proxy chegam la e so falham em autenticacao. So o
loopback escapa, porque o Qt o trata a parte.

O socket nasce com DefaultProxy, o Qt escolhe o motor de socket direto por
causa da resposta NoProxy, e na hora de conectar o DefaultProxy vira o
applicationProxy, que e o proxy do sistema; o motor direto so aceita NoProxy
ou DefaultProxy e recusa pelo tipo. A mensagem vem da tabela de erros do
QNativeSocketEngine.
...
desenhando. O que nao funciona e deixar o proxy do QGIS em branco.
"""
...
def fetchWithOwnManager(url, timeout=30000):
    """
    Baixa a URL com um QNetworkAccessManager proprio e proxy resolvido a mao.
    Devolve (corpo, mensagem_de_erro).
    """
...
def adviseProxyConfiguration(tr):
    """
    Avisa, uma vez por sessao, que a maquina precisa do proxy preenchido nas
    Opcoes do QGIS. Sem isso as camadas entram sem erro e nao desenham,
