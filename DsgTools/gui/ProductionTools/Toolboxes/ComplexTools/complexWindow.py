...
# DsgTools imports
from DsgTools.gui.ProductionTools.Toolboxes.ComplexTools.manageComplex import (
    ManageComplexDialog,
)
...
FORM_CLASS, _ = uic.loadUiType(
    os.path.join(os.path.dirname(__file__), "complexWindow_base.ui")
)
...
class ComplexWindow(QtWidgets.QDockWidget, FORM_CLASS):
    ...
    def getUserCredentials(self, lyr):
        ...
        (success, user, passwd) = QgsCredentials.instance().get(
            connInfo, dataSourceUri.username(), None
        )
    ...
    def on_managePushButton_clicked(self):
        ...
        if not self.abstractDb:
            ...
            return
        dlg = ManageComplexDialog(
            self.iface, self.abstractDb, self.complexCombo.currentText()
        )
    ...
    def on_zoomButton_clicked(self):
        ...
        if len(self.treeWidget.selectedItems()) == 0:
            QMessageBox.warning(
                self.iface.mainWindow(),
                self.tr("Warning!"),
                self.tr("Please, select an item to zoom."),
            )
            return
            ...
            for i in range(item.childCount()):
                ...
                if not aggregated_layer:
                    QMessageBox.warning(
                        ...
                        self.tr("Warning!"),
                        self.tr(
                            "The associated classes must be loaded in the table of contents."
                        ),
                    )
                    return
    ...
    def disassociateAggregatedClass(self, item):
        ...
        for layer in layers:
            ...
            if (
                layer.name() == aggregated_class
                or layer.name() == "complexos_" + aggregated_class
            ):
                aggregated_layer = layer
                break
        ...
        if not aggregated_layer:
            QMessageBox.warning(
                ...
                self.tr("Warning!"),
                self.tr(
                    "The class you're trying to disassociate must loaded in the table of contents."
                ),
            )
            return
    ...
    def updateLayerOnDisassociate(self, layer, aggregated_class, link_column, id):
        ...
        try:
            if self.abstractDb.isComplexClass(aggregated_class):
                self.abstractDb.disassociateComplexFromComplex(
                    aggregated_class, link_column, id
                )
            else:
                # field index that will be set to NULL
                fieldIndex = [
                    i
                    for i in range(len(layer.dataProvider().fields()))
                    if layer.dataProvider().fields()[i].name() == link_column
                ]
    ...
    def on_disassociatePushButton_clicked(self):
        ...
        if len(self.treeWidget.selectedItems()) == 0:
            QMessageBox.warning(
                self.iface.mainWindow(),
                self.tr("Warning!"),
                self.tr("Please, select an aggregated class or aggregated id."),
            )
            return
            ...
            for complex_uuid in list(associatedDict[name].keys()):
                ...
                for aggregated_class in associatedDict[name][complex_uuid]:
                    for ogc_fid in associatedDict[name][complex_uuid][aggregated_class]:
                        self.addAssociatedFeature(
                            complex, name, complex_uuid, aggregated_class, ogc_fid
                        )
    ...
    def associateFeatures(self):
        ...
        if len(forbiddenLayers) > 0:
            message = ""
            message += (
                self.tr("The following layers cannot be associated to complexes from ")
    ...
    def addAssociatedFeature(
        self, className, complexName, complexId, associatedClass, associatedId
    ):
        """
        ...
        classNameItem = self.createTreeItem(
            self.treeWidget.invisibleRootItem(), className
        )
